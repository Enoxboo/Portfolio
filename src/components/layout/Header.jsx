...
const Header = () => {
    ...
    const navLinks = [{name: 'Accueil', href: '#hero'}, {name: 'À propos', href: '#about'}, {
        name: 'Projets', href: '#projects'
    }, {name: 'Contact', href: '#contact'},];

    return (<header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-dark/95 backdrop-blur-sm shadow-lg' : 'bg-transparent'}`}
    >
        <nav className="container mx-auto px-6 py-4">
            ...
            {/* Mobile Navigation */}
            {isMobileMenuOpen && (<ul className="md:hidden mt-4 pb-4 space-y-4">
                {navLinks.map((link) => (<li key={link.name}>
                    <a
                        href={link.href}
